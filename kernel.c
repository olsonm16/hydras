...
/*
executeProgram takes a file name and segment.
The segment is checked to ensure it is a multiple of 0x1000 and not 0x1000 itself or beyond 0xA000.
If the segment is not in those restrictions, -2 is returned.
Otherwise, file name is read into a buffer using readfile.
...
int executeProgram(char * name, int segment) {
	...
	if (mod(segment, 0x1000) != 0) { return -2;};

	if (segment == 0x1000) {return -2;};
	if (segment >= 0xA000) {return -2;};
	
	sector = readFile(name, buffer);
...
void handleTimerInterrupt(int segment, int stackPointer) {
	...
	//and then call the kernel.asm returnFromTimer
